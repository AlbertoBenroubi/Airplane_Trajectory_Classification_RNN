...
class VariableSequenceClassification:
    ...
    def __init__(self, data, target, num_hidden=420, num_layers=1):
        self.data = data
        self.target = target
    ...
    for i in range(14):
        ...
        while step * batch_size < training_iters:
            ...
            y_final=y_final.reshape([batch_size, n_classes])
     	    ...
     	    sess.run(model.optimize, {data: x_final, target: y_final})
            #print sess.run(model.length,{data: x_final, target: y_final} )
            print sess.run(model.accuracy,{data: x_final, target: y_final} )
            if step % 10 ==0:
                accuracy_sum=0
                for k in range(20):
                ...
                print('Epoch {:2d} step {:2d} accuracy {:3.1f}%'.format(i, step + 1, 100 * mean_accuracy))
                if mean_accuracy>max_acc:
                    max_acc=mean_accuracy
                    print(saver.save(sess, "output/model.ckpt"))
            #if step % 50 ==0:
        	#	send_email('email','password','sendTo','Training process','Epoch {:2d} step {:2d} accuracy {:3.1f}%  max accuracy {:3.1f}%'.format(i, step + 1, 100 * mean_accuracy,100*max_acc))
